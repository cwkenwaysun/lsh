...
def storage(storage_config, index):
    ...
    elif 'mongo' in storage_config:
        db = storage_config['mongo']['db']
        table = db['table' + str(index)].find()[0]
        table.pop('_id')
        tuple_table = {}
        for key in table:
            tuple_table[key] = to_tuple(table[key])
        #print(table)
        #print(tuple_table)
        return MongoStorage(tuple_table)
    else:
        raise ValueError("Only in-memory dictionary, pymongo and Redis are supported.")
